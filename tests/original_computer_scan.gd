...
func run():
	...
	check(scene.native.load_session("res://tests/natural-blocked-fort.json").is_empty(),"load natural Cao Cao insufficient-mobility boundary")
	scene.refresh();scene.show_battle_details()
	check(not scene.computer_next.disabled and scene.computer_next.text=="检查下一部队","natural blocked-fort entry offers next unit")
	scene.computer_next.pressed.emit()
	check(int(scene.state.battle.tactics.points)==3 and scene.state.battle.tactics.computer_plan.get("blocked",false),"rejected fort move keeps three mobility points")
	check(scene.battle_description.text.contains("未能补入城堡") and not scene.computer_next.disabled,"report explains unchanged position and permits continuation")
	reload_step()
	scene.computer_next.pressed.emit()
	check(scene.state.battle.tactics.moves.back().kind=="computer_tactics","next assessment runs after rejected movement")
